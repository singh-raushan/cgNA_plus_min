function [curshapes] = nondim2curPeriodic(shapes)
...
ncay  = sqrt(sum(cay.^2,2));
angle = 2*atan(ncay)/pi*180;
etapWc    = bsxfun(@times, angle, bsxfun(@rdivide, cay, ncay));

cay   = etapC/10;
ncay  = sqrt(sum(cay.^2,2));
angle = 2*atan(ncay)/pi*180;
etapCc    = bsxfun(@times, angle, bsxfun(@rdivide, cay, ncay));

curshapes = shapes2vectorPeriodic(etac, w, etapWc, wpW, uc, v, etapCc, wpC);
